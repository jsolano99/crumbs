...
- **"How to implement it" honesty fix**: the model was padding this section with generic non-actionable filler ("research this," "look up that") for tweets that are pure news/analysis, not real how-tos. The prompt now requires the model to decide which kind of tweet it is and, when there's genuinely nothing to implement, write one honest sentence instead of manufacturing fake steps.
